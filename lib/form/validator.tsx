...
let Validator: (formValue: FormValue, rules: FormRules, callback: (errors: any) => void) => void;
Validator = (formValue: FormValue, rules: FormRules, callback: (errors: any) => void): void => {
  let errors: any = {};
  ...
  Promise.all(newPromises).then(results => {
    callback(zip((results.filter(item => item[1]) as Array<[string, string]>)));
  });
};
